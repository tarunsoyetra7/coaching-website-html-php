...
#amnt_res{
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
			...
			$editQ=$db->query("SELECT id,frm_emp_id ,send_date ,recive_date ,amount ,remark 
			FROM recipt_master WHERE flag='true' AND id=$id ORDER BY id desc") or die("");	

			$edit_q_res=$editQ->fetch(PDO::FETCH_ASSOC);
			
						 ?>
						  <form role="form" name="" id="txtFrm" method="post" enctype="multipart/form-data"  action="recived_amount_master_do.php">
						...
						<select name="txt_frm_emp_id" id="txt_frm_emp_id" class="form-control">
							...
							<?php
							$empQ=$db->query("SELECT user_id,user_name FROM user_infromation WHERE user_id!=$login_id") or die("");
							while($empQ_res=$empQ->fetch(PDO::FETCH_ASSOC)){
								if($empQ_res['id']==$edit_q_res['frm_emp_id']){
									?>
									<option selected value="<?php echo $empQ_res['user_id']; ?>">
								<?php echo $empQ_res['user_name']; ?>
							</option>
									<?php
								}
								else{
							?>
							<option value="<?php echo $empQ_res['user_id']; ?>">
								<?php echo $empQ_res['user_name']; ?>
							</option>
								<?php } ?>
							<?php } ?>
