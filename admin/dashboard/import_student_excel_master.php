...
$("#txtSelCourse").on("change",function(e){
	...
	if($("#txtSelCourse option:selected").val()=="s_c"){
		...
		var course_val=$("#txtSelCourse option:selected").val();
		
			$.ajax({
				type:"POST",
				data:{course_val:course_val},
				url:"check_stud_in_course.php",
				success: function(r_data){
					if(r_data=="no_stud"){
					}
					else{
						alert("Students are already added in this course !...");
						$('#txtSelCourse option[value="s_c"]').attr('selected','selected');
					}
				},error:function(err){
					location.reload();
				}
