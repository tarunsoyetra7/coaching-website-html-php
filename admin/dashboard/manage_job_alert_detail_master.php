    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
			<div class="container-fluid">
				<div class="row">
                    <div class="col-lg-12">
						<h4 align="center"><strong>Menu Job Alert Detail</strong></h4><hr>
                    ...
                    <div class="col-lg-12">
					 ...
					 <div class="col-lg-6">
                                    ...
                                    <label>Select Job Alert Category</label>
...
<?php } ?>
 ...
 <table id="example1" class="table table-middle dataTable table-bordered table-condensed table-hover">
                            ...
                            <tbody id="filter_Res">
							<?php $query=$db->query("select id,job_title,job_cat_id,created_on,
							e_d_optn,flag from job_alert_detail order by id desc") or die("");
 $i=0;
while($result=$query->fetch(PDO::FETCH_ASSOC)){			$i++;				?>
							<tr>
								...
								<td>
								<?php $cat_id=$result['job_cat_id'];
								
										$catQ=$db->query("select group_concat(cat_title) as cat_title 
										from job_alert_category_master where find_in_set(id,'$cat_id')") or die("");
...
$("#sel_menu_cat").on("change",function(e){
	if($("#sel_menu_cat option:selected").val()=="s_m"){		
		...
		$.ajax({
			type:"POST",
			url:"filter_job_alert_category_detail.php",
...
<script>
  $(".btn_edit").on("click",function(e){
		...
		window.location.replace("job_alert_detail_master.php?id="+cur_id);
	});
...
$(".btn_delete").on("click",function(e){
		...
		$.ajax({
			type:"POST",
			url:"delete_job_alert_detail_master.php",
			data:{cur_id:cur_id},
			success:function(r_data){
				alert(r_data);
				location.reload();
...
$(".btn_restore").on("click",function(e){
		...
		$.ajax({
			type:"POST",
			url:"restore_job_alert_detail_master.php",
			data:{cur_id:cur_id},
			success:function(r_data){
				alert(r_data);
				location.reload();
	...
	$(".enable_btn").on("click",function(e){
		...
		$.ajax({
			type:"POST",
			url:"enable_job_alert_detail_master.php",
			data:{cur_id:cur_id},
			success:function(r_data){
				alert(r_data);
				location.reload();
	...
	$(".btn_disable").on("click",function(e){
		...
		$.ajax({
			type:"POST",
			url:"disable_job_alert_detail_master.php",
			data:{cur_id:cur_id},
			success:function(r_data){
				alert(r_data);
				location.reload();
