    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
			<div class="container-fluid">
				<div class="row">
                    <div class="col-lg-12">
						<h4 align="center"><strong>Manage News Article Category Master</strong></h4><hr>
...
FROM news_article_category_master AS node
LEFT OUTER JOIN news_article_category_master AS up1
ON up1.id = node.cat_parent_id
LEFT OUTER JOIN news_article_category_master AS up2
ON up2.id = up1.cat_parent_id
LEFT OUTER JOIN news_article_category_master AS up3
 ...
 ?>
							...
							<td><?php echo $i; ?></td>
								...
								<?php if($result['cat_img']=="" || $result['cat_img']==NULL){ } else { ?>
									<img src="../../news_article_cat_img/<?php echo $result['id'].".".$result['cat_img']; ?>" style="width:30px; height:30px;">
								<?php } ?>
...
<script type="text/javascript">    
	...
	$(".btn_edit").on("click",function(e){
		...
		window.location.replace("news_article_category_master.php?id="+cur_id);
	});
...
$(".btn_delete").on("click",function(e){
		...
		$.ajax({
			type:"POST",
			url:"delete_news_article_category_master.php",
			data:{cur_id:cur_id},
...
$(".btn_restore").on("click",function(e){
		...
		$.ajax({
			type:"POST",
			url:"restore_news_article_category_master.php",
			data:{cur_id:cur_id},
			success:function(r_data){
				alert(r_data);
				location.reload();
	...
	$(".enable_btn").on("click",function(e){
		...
		$.ajax({
			type:"POST",
			url:"enable_news_article_category_master.php",
			data:{cur_id:cur_id},
			success:function(r_data){
				alert(r_data);
				location.reload();
	...
	$(".btn_disable").on("click",function(e){
		...
		$.ajax({
			type:"POST",
			url:"disable_news_article_category_master.php",
			data:{cur_id:cur_id},
			success:function(r_data){
				alert(r_data);
				location.reload();
