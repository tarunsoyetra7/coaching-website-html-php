            ...
            <?php  include("header.php");  ?>
                            ...
                            <div id="page-wrapper">
                                <div class="container-fluid">
                                    <div class="row">
                                        <div class="col-lg-12">
                                            <h4 align="center"><strong>Job Alert  Detail</strong></h4>
                                            <hr>
...
$editQ=$db->query("select id,job_title,job_cat_id,job_short_img,job_s_descp,job_doc,job_l_descp,job_key,
job_descp from job_alert_detail where flag='true' and id=$edit_id") or die("");
$editQ_res=$editQ->fetch(PDO::FETCH_ASSOC);

?>

<form method="post" action="job_alert_detail_master_do.php" enctype="multipart/form-data" id="txt_form">
                                            <div class="col-lg-8">

			<label>Title</label>
			<input id="txt_title" name="txt_title" type="text" class="form-control" placeholder="Title *" value="<?php echo $editQ_res['job_title']; ?>">
			<input type="hidden" name="txt_hide" id="txt_hide" value="<?php echo $editQ_res['id']; ?>">
			<br>
			<label>Short Description</label>
			<textarea rows="4" id="txt_short_description" id="txt_short_description" name="txt_short_description" class="form-control" placeholder="Short Description *"><?php echo $editQ_res['job_s_descp']; ?></textarea>
			<br>
			
			
			<label>Short Image</label>
			<input type="file" name="txt_short_img" id="txt_short_img" class="form-control">
			<?php if($editQ_res['job_short_img']=="" || $editQ_res['job_short_img']==NULL){ } else { ?>
			<img src="../../job_alert_short_img/<?php echo $editQ_res['id'].".".$editQ_res['job_short_img'];  ?>" style="width:30px; height:30px;">
			<?php } ?>
			...
			<main>
                                                    ...
                                                    <div id="editor"><?php echo $editQ_res['job_l_descp']; ?></div>
                                                </main>
			...
			<div class="col-lg-6" style="padding-left:0px;">
				...
				<textarea class="form-control" id="txt_keyword" name="txt_keyword" placeholder="Keyword *" rows="4"><?php echo $editQ_res['job_key']; ?></textarea>
				<br>
			</div>
			<div class="col-lg-6" style="padding-right:0px;">
				<label>Description</label>
				<textarea class="form-control" id="txt_description" name="txt_description" placeholder="Description *" rows="4"><?php echo $editQ_res['job_descp']; ?></textarea>
				<br>
			</div>
...
function parent_child($id){
	...
	$q2=$db->query("select id,cat_title from job_alert_category_master where cat_parent_id=$id") or die("");
...
?> 
											   ...
											   <input type="hidden" id="txtEditCatId" value="<?php echo $editQ_res['job_cat_id']; ?>">
											   
       <?php $q1=$db->query("select id,cat_title from job_alert_category_master where cat_parent_id=0") or die("");
...
<ul id="treeview" class="hummingbird-base list_category_list">
	<?php 
		$q1=$db->query("select id,cat_title from job_alert_category_master where cat_parent_id=0") or die("");
		while($r1=$q1->fetch(PDO::FETCH_ASSOC)){ $m_id=$r1['id']; ?>
		<!--start level 1--->
		<li>
		<label><input type="checkbox" id="<?php echo $m_id; ?>" class="check_cat">
			...
			<ul>
				<?php
				$q2=$db->query("select id,cat_title from job_alert_category_master where cat_parent_id=$m_id") or die("");
				...
				<?php echo $r2['cat_title']; ?></label> 
					<ul>
						<?php
						$q3=$db->query("select id,cat_title from job_alert_category_master where cat_parent_id=$m_id1") or die("query");
						...
						<?php echo $r3['cat_title']; ?> </label>
							<ul>
								<?php
								$q4=$db->query("select id,cat_title from job_alert_category_master where cat_parent_id=$m_id2") or die("query");
