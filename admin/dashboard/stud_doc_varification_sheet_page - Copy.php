...
$fetchCenterDetail=$db->query("SELECT t_c_name,t_cmp_add,o_c_n_name FROM center_info_master WHERE id=$id AND flag='true'") or die("");

$fetchCenterDetail_res=$fetchCenterDetail->fetch(PDO::FETCH_ASSOC);


$c_name=str_replace("&amp;","&",$fetchCenterDetail_res['o_c_n_name']);
...
$fechStudQ=$db->query("SELECT id,stud_enroll_no,stud_image,stud_name,stud_mo,stud_m_name,stud_f_name FROM student_master  WHERE flag='true' AND institut_id=$id ORDER BY id desc") or die("");
