    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
			...
			$edit_q_res=$editQ->fetch(PDO::FETCH_ASSOC);
						 ...
						 ?>
						  <form role="form" name="" id="txtFrm" method="post" enctype="multipart/form-data"  action="installment_master_do.php">
						...
						<select name="txt_frm_emp_id" id="txt_frm_emp_id" class="form-control">
							...
							<?php
							$empQ=$db->query("SELECT id,stud_name FROM student_master ORDER BY id desc") or die("");
...
<button class="btn btn-sm btn-success btn_submit" type="button" disabled>Submit</button>
						...
						<select name="txt_frm_emp_id" id="txt_frm_emp_id" class="form-control">
							...
							<?php
							$empQ=$db->query("SELECT id,stud_name,stud_certificate_no FROM student_master ORDER BY stud_name ASC") or die("");
							while($empQ_res=$empQ->fetch(PDO::FETCH_ASSOC)){
