...
$current_date=date("Y/m/d");
				...
				if($txt_hide=="" || $txt_hide==NULL){		
					...
					$insQ=$db->query("INSERT INTO student_master_1(stud_name,stud_email,stud_add,stud_mo,stud_branch,stud_sem,stud_clg_name,stud_course,stud_fees,
...
$insQ=$db->query("insert into recipt_master_1(reg_stud_id,frm_emp_id ,send_date ,amount ,
...
/*---end email---*/
			...
			<script>
				...
				window.location.replace("student_master_1.php");
			</script>
					<?php
				}
				else{				
						$updateQ=$db->query("UPDATE student_master_1 SET stud_name='$txt_name',
...
$recipt_id_q=$db->query("SELECT recipt_master_1.id as r_id,recipt_master_1.amount as r_amnt,recipt_master_1.pay_type,student_master_1.id,student_master_1.stud_name FROM `recipt_master_1`,student_master_1 where  recipt_master_1.reg_stud_id=$txt_hide and  recipt_master_1.created_by=$login_id and student_master_1.id=recipt_master_1.reg_stud_id order by recipt_master_1.id desc limit 1") or die("");
...
/*$insQ=$db->query("update recipt_master_1 set amount='$txt_paid',updated_on=NOW() ,updated_by=$login_id 
 where reg_stud_id=$txt_hide") or die("");
	...
	*/	
		...
		window.location.replace("manage_student_master_1.php");
			</script>
					<?php
				}	
	}
