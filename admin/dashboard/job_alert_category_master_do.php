		...
		$cur_date=date("Y-m-d");
			if($img_ext=="" || $img_ext==NULL || $img_ext=="jpg" || $img_ext=="JPG" || $img_ext=="png" || 
				...
				if($txt_hide=="" || $txt_hide==NULL){
					...
					$insQ=$db->query("insert into job_alert_category_master(cat_title,cat_parent_id,cat_keyword,cat_descp,
					...
					move_uploaded_file($_FILES['txt_image']['tmp_name'],"../../job_alert_cat_img/".$img_name);
					?>
						<script>
				alert("Sucessfully Added !...");
				window.location.replace("job_alert_category_master.php");
			</script>
					<?php
				}
				else{
					...
					$updateQ=$db->query("update job_alert_category_master set cat_title='$txt_menu_title',
				...
				window.location.replace("manage_job_alert_category_master.php");
			</script>
					<?php
				}	
