...
if(isset($_COOKIE['login']))
{
	...
	if(isset($_REQUEST['txt_frm_emp_id']) && isset($_REQUEST['txt_sel_date']) 
		...
		$stud_tab_update=$db->query("UPDATE student_master SET stud_paid_amnt=stud_paid_amnt+$txt_amnt,stud_balance=stud_balance-$txt_amnt
...
stud_paid_amnt,stud_balance FROM student_master WHERE id=$txt_frm_emp_id") or die("");
...
$stud_email=$fetchQ_res['stud_email'];
...
$recipt_id_q=$db->query("SELECT recipt_master.id as r_id,student_master.id,student_master.stud_name FROM `recipt_master`,student_master where  recipt_master.reg_stud_id=$txt_frm_emp_id and  recipt_master.created_by=$login_id and student_master.id=recipt_master.reg_stud_id order by recipt_master.id desc limit 1") or die("");
...
$recipt_id_q_1=$db->query("SELECT recipt_master.amount as r_amnt FROM `recipt_master`,student_master where  recipt_master.reg_stud_id=$txt_frm_emp_id and  recipt_master.created_by=$login_id and student_master.id=recipt_master.reg_stud_id and recipt_master.id<$poid order by recipt_master.id desc limit 1") or die("");
...
//new end
		...
		?>
			<script>
				...
				window.location.replace("installment_master.php");
			</script>
		<?php
		
		}
		else{
			$insQ=$db->query("update recipt_master set frm_emp_id =5,
