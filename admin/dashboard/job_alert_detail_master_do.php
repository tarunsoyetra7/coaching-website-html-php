		...
		$cur_date=date("Y-m-d");
			if($txt_short_img_ext=="jpg" || $txt_short_img_ext=="JPG" 
				...
				if($txt_hide=="" || $txt_hide==NULL){
					...
					$insQ=$db->query("insert into job_alert_detail(job_title,job_cat_id,job_s_descp,job_l_descp,job_key,
					job_descp,job_short_img,created_by,created_on)
					...
					move_uploaded_file($_FILES['txt_short_img']['tmp_name'],"../../job_alert_short_img/".$short_img_name);
					
					
					?>
						<script>
				alert("Sucessfully Added !...");
				window.location.replace("job_alert_detail_master.php");
			</script>
					<?php
				}
				else{
					...
					if($txt_short_img_ext=="" || $txt_short_img_ext==NULL){
						...
						$short_img_condition=",job_short_img='$txt_short_img_ext'";
						$lastID=$txt_hide;
					$short_img_name=$lastID.".".$txt_short_img_ext;
					move_uploaded_file($_FILES['txt_short_img']['tmp_name'],"../../job_alert_short_img/".$short_img_name);
					}
					...
					$updateQ=$db->query("update job_alert_detail set job_title='$txt_title',job_cat_id='$txt_menu_cat_hide',
					job_s_descp='$txt_short_description'
					
					 $short_img_condition
					
					,job_l_descp='$txt_long_descp',job_key='$txt_keyword',
					job_descp='$txt_description',updated_by=$login_id,updated_on=NOW() where id=$txt_hide") or die("");			
