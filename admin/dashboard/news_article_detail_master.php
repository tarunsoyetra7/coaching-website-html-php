            ...
            <?php  include("header.php");  ?>
                            ...
                            <div id="page-wrapper">
                                <div class="container-fluid">
                                    <div class="row">
                                        <div class="col-lg-12">
                                            <h4 align="center"><strong>News Article  Detail</strong></h4>
                                            <hr>

                                            <ol class="breadcrumb">

                                            <li class="active">
                                        <a href="new_article_category_master.php"><i class="fa fa-dashboard"></i> Add News Article Category</a>
                                    </li>

                                    <li class="active">
                                        <a href="manage_news_atricle_category_master.php"><i class="fa fa-dashboard"></i> Manage News Article Category</a>
                                    </li>
									
									<li class="active">
                                        <a href="news_article_detail_master.php"><i class="fa fa-dashboard"></i> Add News Article Detail</a>
                                    </li>
									
									<li class="active">
                                        <a href="manage_news_article_detail_master.php"><i class="fa fa-dashboard"></i> Manage News Article Detail</a>
                                    </li>
...
$editQ=$db->query("select id,n_title,n_cat_id,n_short_img,n_s_descp,n_doc,n_l_descp,n_key,
n_descp from news_article_detail where flag='true' and id=$edit_id") or die("");
$editQ_res=$editQ->fetch(PDO::FETCH_ASSOC);

?>

<form method="post" action="news_article_detail_master_do.php" enctype="multipart/form-data" id="txt_form">
                                            <div class="col-lg-8">

			<label>Title</label>
			<input id="txt_title" name="txt_title" type="text" class="form-control" placeholder="Title *" value="<?php echo $editQ_res['n_title']; ?>">
			<input type="hidden" name="txt_hide" id="txt_hide" value="<?php echo $editQ_res['id']; ?>">
			<br>
			<label>Short Description</label>
			<textarea rows="4" id="txt_short_description" id="txt_short_description" name="txt_short_description" class="form-control" placeholder="Short Description *"><?php echo $editQ_res['n_s_descp']; ?></textarea>
			<br>
			
			
			<label>Short Image</label>
			<input type="file" name="txt_short_img" id="txt_short_img" class="form-control">
			<?php if($editQ_res['n_short_img']=="" || $editQ_res['n_short_img']==NULL){ } else { ?>
			<img src="../../news_article_short_img/<?php echo $editQ_res['id'].".".$editQ_res['n_short_img'];  ?>" style="width:30px; height:30px;">
			<?php } ?>
			...
			<main>
                                                    ...
                                                    <div id="editor"><?php echo $editQ_res['n_l_descp']; ?></div>
                                                </main>
			...
			<div class="col-lg-6" style="padding-left:0px;">
				...
				<textarea class="form-control" id="txt_keyword" name="txt_keyword" placeholder="Keyword *" rows="4"><?php echo $editQ_res['n_key']; ?></textarea>
				<br>
			</div>
			<div class="col-lg-6" style="padding-right:0px;">
				<label>Description</label>
				<textarea class="form-control" id="txt_description" name="txt_description" placeholder="Description *" rows="4"><?php echo $editQ_res['n_descp']; ?></textarea>
				<br>
			</div>
...
function parent_child($id){
	...
	$q2=$db->query("select id,cat_title from news_article_category_master where cat_parent_id=$id") or die("");
	while($r2=$q2->fetch(PDO::FETCH_ASSOC)){  $id=$r2['id'];
?>
	<ul>
		<li><span><input type="checkbox" id="<?php echo $id; ?>" class="check_cat"></span>
			<?php echo $r2['cat_title']; ?>  
			<?php
				$count=$q2->rowCount();
				if($count!=0){			
				}		 
...
?> 
											   ...
											   <input type="hidden" id="txtEditCatId" value="<?php echo $editQ_res['n_cat_id']; ?>">
											   
       <?php $q1=$db->query("select id,cat_title from news_article_category_master where cat_parent_id=0") or die("");
	  while($r1=$q1->fetch(PDO::FETCH_ASSOC)){ $m_id=$r1['id']; ?>
                                                       <li><span><input type="checkbox" id="<?php echo $m_id; ?>" class="check_cat"></span> 
                                                    <?php echo $r1['cat_title']; ?><span> </span>
...
<ul id="treeview" class="hummingbird-base list_category_list">
	<?php 
		$q1=$db->query("select id,cat_title from news_article_category_master where cat_parent_id=0") or die("");
		while($r1=$q1->fetch(PDO::FETCH_ASSOC)){ $m_id=$r1['id']; ?>
		<!--start level 1--->
		<li>
		<label><input type="checkbox" id="<?php echo $m_id; ?>" class="check_cat">
			<?php echo $r1['cat_title']; ?></label>
			<ul>
				<?php
				$q2=$db->query("select id,cat_title from news_article_category_master where cat_parent_id=$m_id") or die("");
				while($r2=$q2->fetch(PDO::FETCH_ASSOC)){ $m_id1=$r2['id'];
				?>
				<!---start level 2--->
				<li>
				<label><input type="checkbox" id="<?php echo $m_id."-".$m_id1; ?>" class="check_cat">
				<?php echo $r2['cat_title']; ?></label> 
					<ul>
						<?php
						$q3=$db->query("select id,cat_title from news_article_category_master where cat_parent_id=$m_id1") or die("query");
						while($r3=$q3->fetch(PDO::FETCH_ASSOC)){ $m_id2=$r3['id'];
						?>
						<!---start level 3--->
						<li>
						<label><input type="checkbox" id="<?php echo $m_id."-".$m_id1."-".$m_id2; ?>" class="check_cat"> 
						<?php echo $r3['cat_title']; ?> </label>
							<ul>
								<?php
								$q4=$db->query("select id,cat_title from news_article_category_master where cat_parent_id=$m_id2") or die("query");
...
var empty_res_list="";
	$('input:checkbox').change(function(){
		var chk_val_id=$(this).attr('id');
		
			
		chk_val_id=chk_val_id.split("-");
		var empty_val="";
		for(i=0; i<chk_val_id.length; i++){		
			
			if(i==0){
				empty_val=empty_val+chk_val_id[i];				
			}
			else{
				empty_val=empty_val+"-"+chk_val_id[i];
			}
			
			if($("#"+empty_val).hasClass("chk_cat")){				
				$("#"+empty_val).removeClass("chk_cat");
				
			}
			else{				
				$("#"+empty_val).addClass("chk_cat");
				$("#"+empty_val).prop('checked', true);
