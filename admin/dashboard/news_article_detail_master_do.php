		...
		$cur_date=date("Y-m-d");
			if($txt_short_img_ext=="jpg" || $txt_short_img_ext=="JPG" 
				...
				if($txt_hide=="" || $txt_hide==NULL){
					...
					$insQ=$db->query("insert into news_article_detail(n_title,n_cat_id,n_s_descp,n_l_descp,n_key,
					n_descp,n_short_img,created_by,created_on)
					...
					move_uploaded_file($_FILES['txt_short_img']['tmp_name'],"../../news_article_short_img/".$short_img_name);
					
					
					?>
						<script>
				alert("Sucessfully Added !...");
				window.location.replace("news_article_detail_master.php");
			</script>
					<?php
				}
				else{
					...
					if($txt_short_img_ext=="" || $txt_short_img_ext==NULL){
						...
						$short_img_condition=",n_short_img='$txt_short_img_ext'";
						$lastID=$txt_hide;
					$short_img_name=$lastID.".".$txt_short_img_ext;
					move_uploaded_file($_FILES['txt_short_img']['tmp_name'],"../../news_article_short_img/".$short_img_name);
					}
					...
					$updateQ=$db->query("update news_article_detail set n_title='$txt_title',n_cat_id='$txt_menu_cat_hide',
					n_s_descp='$txt_short_description'
					
					 $short_img_condition
					
					,n_l_descp='$txt_long_descp',n_key='$txt_keyword',
					n_descp='$txt_description',updated_by=$login_id,updated_on=NOW() where id=$txt_hide") or die("");			
					
					?>
					<script>
				alert("Sucessfully Updated !...");
				window.location.replace("manage_news_article_detail_master.php");
			</script>
					<?php
				}	
