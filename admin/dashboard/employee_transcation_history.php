    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
			<div class="container-fluid">
				<div class="row">
                    <div class="col-lg-12">
						<h4 align="center"><strong>Transcation History</strong></h4><hr>
...
(SELECT id, 
        ...
        remark ,pay_type
 FROM   recipt_master 
 WHERE  frm_emp_id = $login_id)  ORDER BY  id DESC ") or die(""); $i=0;
			...
			while($result=$query->fetch(PDO::FETCH_ASSOC)){ $i++;
							...
							<tr>
								...
								<td><?php echo $result['receive'];
									...
									if($result['pay_type']=="instal"){ echo "(Installment)"; } 
								?></td>
								...
								<td>
								<?php if($result['pay_type']=="instal"){
?>
<button id="<?php echo $result['id']; ?>" class="btn btn-sm btn-info btn_edit_instal">Edit</button>
<?php
								} else{ ?>
		...
		<b>Total Receive: </b>
				<u><?php echo $total_receive; ?></u><br>
				<b>Total Given: </b><u><?php echo $total_given; ?></u><br>	
			<b>Amount in Hand : </b><u><?php echo $total_receive-$total_given ; ?></u><br>
                            </tbody>
...
$(document).ready(function() {
	...
	$(".btn_edit_instal").on("click",function(e){
		var cur_id=$(this).attr('id');
		window.location.replace("installment_master.php?id="+cur_id);
	});
