		...
		$cur_date=date("Y-m-d");
			if($img_ext=="" || $img_ext==NULL || $img_ext=="jpg" || $img_ext=="JPG" || $img_ext=="png" || 
				...
				if($txt_hide=="" || $txt_hide==NULL){
					...
					$insQ=$db->query("insert into current_affair_category_master(cat_title,cat_parent_id,cat_keyword,cat_descp,
					...
					move_uploaded_file($_FILES['txt_image']['tmp_name'],"../../current_affair_cat_img/".$img_name);
					?>
						<script>
				alert("Sucessfully Added !...");
				window.location.replace("current_affair_category_master.php");
			</script>
					<?php
				}
				else{
					...
					$updateQ=$db->query("update current_affair_category_master set cat_title='$txt_menu_title',
				...
				window.location.replace("manage_current_affair_category_master.php");
			</script>
					<?php
				}	
